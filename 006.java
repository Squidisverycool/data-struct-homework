...
public class BSTRecursive {
	...
	public int[] nonAVLNodes(BSTNode n) {
	    ...
	    lastPos = -1; // reset last position
	    Arrays.fill(temp, 0); // reset temp array
	    collectNonAVLNodes(n);
	    Arrays.sort(temp); // sort the array
	    return temp;
	}

	private void collectNonAVLNodes(BSTNode n) {
	    if (n == null) {
	        return;
	    }
	    if (!isAVL(n)) {
	        if (lastPos < temp.length - 1) {
	            temp[++lastPos] = n.data;
	        }
	    }
	    collectNonAVLNodes(n.left);
	    collectNonAVLNodes(n.right);
	}
